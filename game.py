...
class StartScreen:
    def __init__(self, game):
        ...
        self.fontsize = 30
        self.font = pygame.freetype.Font("assets/fonts/Diphylleia-Regular.ttf",
                                         self.fontsize)
        self.run()
    ...
    def run(self):
        while self.startscreen_running:
            ...
            text(self, "press any key to continue", self.font, (255, 255, 255),
...
class Food:
    def __init__(self, game):
        ...
        self.size = 10
        self.x = random.randint(self.game.frame_size + self.size,
                                WIDTH - self.game.frame_size - self.size)
        self.y = random.randint(self.game.frame_size + self.size,
                                HEIGHT - self.game.frame_size - self.size)
...
class Game:
    ...
    def draw_frame(self):
        pygame.draw.rect(self.screen, self.frame_color,
                         (0, 0, WIDTH, HEIGHT), self.frame_size)

    def check_collision(self, Snake):
        if Snake.x <= self.frame_size + (Snake.size / 2) or \
            Snake.x >= WIDTH - self.frame_size - (Snake.size / 2) or \
            Snake.y <= self.frame_size + (Snake.size / 2) or \
                Snake.y >= HEIGHT - self.frame_size - (Snake.size / 2):
            print("Game over!")
            self.running = False
            pygame.quit()
            quit()
    ...
    def check_food_collision(self, Snake, Food):
        if Snake.x - (Snake.size/2) <= Food.x <= Snake.x + (Snake.size/2) and \
                Snake.y - (Snake.size/2) <= Food.y <= Snake.y + (Snake.size/2):
            print("Food eaten!")
            Snake.length += 1
            Food.x = random.randint(self.frame_size + (Food.size / 2),
                                    WIDTH - self.frame_size - (Food.size / 2))
            Food.y = random.randint(self.frame_size + (Food.size / 2),
                                    HEIGHT - self.frame_size - (Food.size / 2))

    def draw_score(self, Snake):
        text(self, "score: " + str(Snake.length - 1),
             self.font, (255, 255, 255),
             self.frame_size + self.fontsize * 2,
             self.frame_size + self.fontsize - 10)
